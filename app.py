...
def init_db():
    ...
    try: exec_sql(conn, "ALTER TABLE records ADD COLUMN phone TEXT")   # 📞 add phone column
    except Exception: pass
    return conn
...
# Form Page
    ...
    with st.form("health_form"):
        ...
        phone=st.text_input("📱 Phone Number (with country code, e.g. +14155552671)")  # 📞 new input
...
"""
            ...
            (name,email,phone,created_at,age,height_cm,weight_kg,bmi,
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (name,email,phone,datetime.utcnow().isoformat(),age,height,weight,bmi,
...
# Dashboard
    ...
    if df.empty:
        ...
        st.dataframe(df[["created_at","name","email","phone","age","bmi","rule_band","ml_band","ml_confidence"]])  # 📞 show phone

        # >>> n8n: Send remedies email & phone call
        st.markdown("#### 📩 Remedies Email / Call")
        col1, col2 = st.columns([1,2])
        with col1:
            send_clicked = st.button("Get Remedies & Notify", use_container_width=True)
        ...
        def _send_latest_to_n8n(latest_row):
            ...
            if not webhook_url:
                ...
                return
            if not latest_row.get("email") and not latest_row.get("phone"):
                st.error("No email or phone found for this record.")
                return
            ...
            payload = {
                ...
                "phone": latest_row.get("phone"),   # 📞 send phone to n8n
                ...
                "llm_task": "Generate a personalized wellness & remedies plan based on risk and features. Send email + prepare text for Twilio phone call."
            }

            with st.spinner("📨 Contacting n8n to generate & send remedies..."):
            ...
            if ok:
                ...
                st.success("📧 Email/SMS/Call sent via n8n")
                if plan_text:
